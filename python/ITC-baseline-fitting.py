...
class VPITC(Instrument):
    ...
    def __init__(self):
        ...
        self.V0 = self.V0 - 0.044 * Units.ml
        return
    ...
    * time record of applied power and temperature difference
    ...
    def fit_baseline(self):
        ...
        def Get_eNegX_Coefficients(x, y, p_guess):
            ...
            [p, cov, infodict, mesg, ier] = scipy.optimize.leastsq(
                _eNegX_residuals, p_guess, args=(x, y), full_output=1)  # works with EPD 7.0 scipy
    ...
    def integrate_heat(self):
        ...
        for injection in self.injections:
            ...
            # (self.differential_power[first_index:(last_index+1)] -
            # self.reference_power +
            # self.baseline_power[first_index:(last_index+1)]).sum()
            excess_energy_input = injection['filter_period'] * (
        ...
        for injection in self.injections:
            string += "%16d %24.3f %24.3f %24.3f %24.3f %24.3f\n" % (injection['number'],
                                                                     ...
                                                                     injection[
                                                                         'spacing'] / Units.s,
                                                                     injection[
                                                                         'filter_period'] / Units.s,
                                                                     injection['evolved_heat'] / Units.ucal)
    ...
    def write_integrated_heats(self, filename):
        ...
        for (n, injection) in enumerate(self.injections):
            ...
            # Form string.
            string += "%12.5f %5.1f %12.5f %12.5f %12.5f %12.5f\n" % (injection['evolved_heat'] / Units.ucal, injection[
                                                                      'volume'] / Units.ul, Pn / Units.mM, Ln / Units.mM, PLn / Units.mM, NDH)
    ...
    def plot(self, filename=None, model=None):
        ...
        # Plot baseline fit.
        pylab.plot(self.filter_period_end_time / Units.s,
                   self.baseline_power / (Units.ucal / Units.s),
                   'g-')
        
        # Plot differential power.
        pylab.plot(self.filter_period_end_time / Units.s,
                   self.differential_power / (Units.ucal / Units.s),
                   'k.',
        ...
        injection_end_times = numpy.zeros(
            [len(self.injections)], numpy.float64)
    ...
    def plot_baseline(self, filename=None):
        ...
        # Plot differential power.
        indices = list(set(range(len(self.differential_power))) -
...
def analyze(name, experiment):
    ...
    # Plot the raw measurements of differential power versus time and the
    # enthalpogram.
    import pylab
    pylab.figure()
    ...
    # plot baseline fit
    pylab.hold(True)
    pylab.plot(experiment.filter_period_end_time / Units.s,
               experiment.baseline_power / (Units.ucal / Units.s),
               'g-')  # plot baseline fit

    # differential power
    pylab.plot(experiment.filter_period_end_time / Units.s,
               experiment.differential_power / (Units.ucal / Units.s),
               'k.',
               markersize=1)
...
def buildModel(experiment):
    ...
    kB = Na * 1.3806504e-23 / 4184.0 * Units.kcal / \
        Units.mol / Units.K  # Boltzmann constant (kcal/mol/K)
    ...
    # Tellinghuisen volume correction for VP-ITC (L) # not sure for iTC-200
    V0 = V0 - 0.044 * Units.ml
    ...
    # duration_n[n] is the duration of injection n
    duration_n = numpy.zeros([N], numpy.float64)
    for n in range(N):
        duration_n[n] = experiment.injections[n]['duration']
    sigma2 = injection_heats[N - nlast:N].var() / duration_n[N - nlast:N].sum()
    # cal/s # TODO: Use std of individual filtered measurements instead
     log_sigma_guess = numpy.log(
         numpy.sqrt(
             sigma2 /
             Units.cal ** 2 *
             Units.second))
    ...
    try:
        ...
        DeltaH_0_guess = - \
            injection_heats[N - 1] / \
            (Ls_stated * experiment.injections[N - 1]['volume'])
        DeltaH_guess = - \
            (injection_heats[0] /
             (Ls_stated *
              experiment.injections[0]['volume']) -
                DeltaH_0_guess)
    except:
        DeltaG_guess = 0.0
        DeltaH_0_guess = 0.0
    ...
    sigma2 = (
        experiment.differential_power[
            first_index:last_index] - experiment.baseline_power[
            first_index:last_index]).var() / filter_time
    print sigma2
    # cal/s # TODO: Use std of individual filtered measurements instead
    log_sigma_guess = numpy.log(
        numpy.sqrt(
    ...
    if (P0_stated > 0.0):
        model['P0'] = pymc.Lognormal('P0',
                                     mu=numpy.log(P0_stated),
                                     tau=1.0 /
                                     numpy.log(1.0 + (dP0 / P0_stated) ** 2),
                                     value=P0_stated)  # true cell concentration (M)
    else:
        model['P0'] = zero

    if (Ls_stated > 0.0):
        model['Ls'] = pymc.Lognormal('Ls',
                                     mu=numpy.log(Ls_stated),
                                     tau=1.0 /
                                     numpy.log(1.0 + (dLs / Ls_stated) ** 2),
                                     value=Ls_stated)  # true syringe concentration (M)
    else:
        model['Ls'] = zero

    model['log_sigma'] = pymc.Uniform(
        'log_sigma',
        lower=log_sigma_min,
        upper=log_sigma_max,
        value=log_sigma_guess)  # natural logarithm of std dev of integrated injection heat divided by 1 cal
    model['DeltaG'] = pymc.Uniform(
        'DeltaG',
        lower=DeltaG_min,
        upper=DeltaG_max,
        value=DeltaG_guess)  # DeltaG (kcal/mol)
    model['DeltaH'] = pymc.Uniform(
        'DeltaH',
        lower=DeltaH_min,
        upper=DeltaH_max,
        value=DeltaH_guess)  # DeltaH (kcal/mol)
    model['DeltaH_0'] = pymc.Uniform(
        'DeltaH_0',
        lower=DeltaH_0_min,
        upper=DeltaH_0_max,
        value=DeltaH_0_guess)  # heat of mixing and mechanical injection (cal/volume)
    ...
    @pymc.deterministic
    def expected_injection_heats(
    DeltaG=model['DeltaG'],
    DeltaH=model['DeltaH'],
    DeltaH_0=model['DeltaH_0'],
    P0=model['P0'],
     Ls=model['Ls']):
        """
        ...
        # Compute dilution factor for instantaneous injection model
        # (perfusion).
        # d_n[n] is the dilution factor for injection n
        d_n = numpy.zeros([N], numpy.float64)
        # dcum_n[n] is the cumulative dilution factor for injection n
        dcum_n = numpy.ones([N], numpy.float64)
        if debug: print "%5s %24s %24s" % ('n', 'd_n', 'dcum_n')
        for n in range(N):
            # dimensionless dilution factor for injection n
            d_n[n] = 1.0 - (experiment.injections[n]['volume'] / V0)
            dcum_n[n:] *= d_n[n]
        ...
        for n in range(N):
            ...
            PLn[n] = 0.5/V0 * \
                ((P + L + Kd*V0) - numpy.sqrt((P + L + Kd*V0)**2 - 4*P*L));
        ...
        q_n = numpy.zeros([N], numpy.float64)
        q_n[0] = (-DeltaH) * V0 * (PLn[0] - d_n[0]*0.0) + (-DeltaH_0) * \
                  experiment.injections[0]['volume'] # first injection
        for n in range(1,N):
            q_n[n] = (-DeltaH) * V0 * (PLn[n] - d_n[n]*PLn[n-1]) + (-DeltaH_0) * \
                      experiment.injections[n][
                          'volume'] # subsequent injections
    ...
    def tau(log_sigma=model['log_sigma']):
        ...
        return numpy.exp(-2.0*log_sigma)/ \
                         (Units.cal**2 / Units.second) * numpy.sqrt(duration_n)
    ...
    model['q_n'] = pymc.Normal(
    'q_n',
    mu=expected_injection_heats,
    tau=tau,
    observed=True,
     value=injection_heats)
...
class RescalingStep(pymc.StepMethod):
   ...
   def propose(self):
      ...
      self.stochastic_index['Ls'].value = self.stochastic_index[
          'Ls'].value * factor
      self.stochastic_index['P0'].value = self.stochastic_index[
          'P0'].value * factor
      self.stochastic_index['DeltaH'].value = self.stochastic_index[
          'DeltaH'].value / factor
      self.stochastic_index['DeltaG'].value = self.stochastic_index[
          'DeltaG'].value + (1./self.beta) * numpy.log(factor);
      self.stochastic_index[
          'DeltaH_0'].value = self.stochastic_index['DeltaH_0'].value
...
if __name__ == "__main__":
    ...
    model['DeltaH_0']],
     model['beta'])
        ...
        pymc.Matplot.plot(
            mcmc.trace('DeltaG')[:] / (Units.kcal/Units.mol), '%s-DeltaG' % name)
        pymc.Matplot.plot(
            mcmc.trace('DeltaH')[:] / (Units.kcal/Units.mol), '%s-DeltaH' % name)
        pymc.Matplot.plot(
            mcmc.trace('DeltaH_0')[:] / (Units.ucal/Units.ul), '%s-DeltaH_0' % name)
        pymc.Matplot.plot(
    numpy.exp(
        mcmc.trace('log_sigma')[:]) *
        Units.cal /
        Units.second**0.5,
        '%s-sigma' %
         name)
        ...
        experiment.plot(
    model=mcmc,
    filename='sampl4/%s-enthalpogram.png' %
     name)
        
        # Compute confidence intervals in thermodynamic parameters.
        outfile = open('sampl4/confidence-intervals.out','a')
        outfile.write('%s\n' % name)
        [x, dx, xlow, xhigh] = compute_statistics(
            mcmc.trace('DeltaG')[:] / (Units.kcal/Units.mol))
        outfile.write(
    'DG:     %8.2f +- %8.2f kcal/mol     [%8.2f, %8.2f] \n' %
     (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(
            mcmc.trace('DeltaH')[:] / (Units.kcal/Units.mol))
        outfile.write(
    'DH:     %8.2f +- %8.2f kcal/mol     [%8.2f, %8.2f] \n' %
     (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(
            mcmc.trace('DeltaH_0')[:] / (Units.ucal/Units.ul))
        outfile.write(
    'DH0:    %8.2f +- %8.2f ucal         [%8.2f, %8.2f] \n' %
     (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(
            mcmc.trace('Ls')[:] / Units.uM)
        outfile.write(
    'Ls:     %8.2f +- %8.2f uM           [%8.2f, %8.2f] \n' %
     (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(
            mcmc.trace('P0')[:] / Units.uM)
        outfile.write(
    'P0:     %8.2f +- %8.2f uM           [%8.2f, %8.2f] \n' %
     (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(
            numpy.exp(mcmc.trace('log_sigma')[:]) * Units.cal / Units.second**0.5)
        outfile.write(
    'sigma:  %8.5f +- %8.5f ucal/s^(1/2) [%8.5f, %8.5f] \n' %
     (x, dx, xlow, xhigh))
        outfile.write('\n')
        outfile.close()
