...
def analyze(name, experiment, output_filename):
...
#    pylab.axis([xmin, xmax, ymin, ymax]) 
    ...
    # show plot
    #pylab.show()
    pylab.savefig(output_filename, orientation='landscape', papertype='letter', format='pdf')
...
def buildModel(experiment):
    ...
    # Correct experiment volume for stirring paddle displacement
    ...
    #dP0 = 0.10 * P0_stated # uncertainty in protein stated concentration (M) - host - best guess
    #dLs = 0.01 * Ls_stated # uncertainty in ligand stated concentration (M) - guests - from mass
    
    dP0 = 0.0174 * P0_stated # uncertainty in protein stated concentration (M) - from gravimetric/volumetric preparation
    dLs = 0.0049 * Ls_stated # uncertainty in ligand stated concentration (M) - from gravimetric/volumetric preparation
    ...
    DeltaH_guess = injection_heats[0] / (Ls_stated * experiment.injections[0]['volume'])
    DeltaH_0_guess = injection_heats[N-1] 

    # DEBUG: For HDR protocol    
    DeltaH_0_guess = injection_heats[0] 
    DeltaH_guess = injection_heats[2] / (Ls_stated * experiment.injections[2]['volume'])
    ...
    model['DeltaH_0'] = pymc.Uniform('DeltaH_0', lower=DeltaH_0_min, upper=DeltaH_0_max, value=DeltaH_0_guess) # heat of mixing and mechanical injection (cal/injection)
    model['P_purity'] = pymc.Uniform('P_purity', lower=0.95, upper=1.0, value=0.975) # purity of protein # TODO: Allow purity range to be specified 
    model['L_purity'] = pymc.Uniform('L_purity', lower=0.95, upper=1.0, value=0.975) # purity of ligand # TODO: Allow purity range to be specified

    @pymc.deterministic
    def expected_injection_heats(DeltaG=model['DeltaG'], DeltaH=model['DeltaH'], DeltaH_0=model['DeltaH_0'], P0=model['P0']*model['P_purity'], Ls=model['Ls']*model['L_purity']):
        """
        ...
        q_n[0] = (-DeltaH) * V0 * (PLn[0] - d_n[0]*0.0) + (-DeltaH_0) # first injection
        for n in range(1,N):
            q_n[n] = (-DeltaH) * V0 * (PLn[n] - d_n[n]*PLn[n-1]) + (-DeltaH_0) # subsequent injections
...
def process_datafile(filename):
        ...
        print experiment
        output_filename = os.path.join(output_directory, name + '.pdf')
        analyze(name, experiment, output_filename)

        # Write Origin-style integrated heats.
        filename = output_directory + '/' + name + '-integrated.txt'
        experiment.write_integrated_heats(filename)
        
        # Write baseline fit information.
        filename = output_directory + '/' + name + '-baseline.png'
        ...
        pymc.Matplot.plot(mcmc.trace('Ls')[:] / Units.uM, output_directory + '/' + '%s-Ls' % name)
        pymc.Matplot.plot(mcmc.trace('P0')[:] / Units.uM, output_directory + '/' + '%s-P0' % name)
        pymc.Matplot.plot(mcmc.trace('L_purity')[:], output_directory + '/' + '%s-Lpurity' % name)
        pymc.Matplot.plot(mcmc.trace('P_purity')[:], output_directory + '/' + '%s-Ppurity' % name)
        pymc.Matplot.plot(mcmc.trace('DeltaG')[:] / (Units.kcal/Units.mol), output_directory + '/' + '%s-DeltaG' % name)
        pymc.Matplot.plot(mcmc.trace('DeltaH')[:] / (Units.kcal/Units.mol), output_directory + '/' + '%s-DeltaH' % name)
        pymc.Matplot.plot(mcmc.trace('DeltaH_0')[:] / Units.ucal, output_directory + '/' + '%s-DeltaH_0' % name)
        pymc.Matplot.plot(numpy.exp(mcmc.trace('log_sigma')[:]) * Units.cal / Units.second**0.5, output_directory + '/' + '%s-sigma' % name)
        
        # TODO: Plot fits to enthalpogram.
        filename = output_directory + '/' + '%s-enthalpogram.png' % name
        experiment.plot(model=mcmc, filename=filename)
        
        # Compute confidence intervals in thermodynamic parameters.
        filename = output_directory + '/' + '%s-confidence-intervals.out'
        outfile = open(filename,'w')
        outfile.write('%s\n' % name)        
        ...
        [x, dx, xlow, xhigh] = compute_statistics(mcmc.trace('DeltaH_0')[:] / Units.ucal)         
        ...
        [x, dx, xlow, xhigh] = compute_statistics(mcmc.trace('L_purity')[:])
        outfile.write('Lpurity:%8.2f +- %8.2f %%            [%8.2f, %8.2f] \n' % (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(mcmc.trace('P0')[:] / Units.uM)         
        outfile.write('P0:     %8.2f +- %8.2f uM           [%8.2f, %8.2f] \n' % (x, dx, xlow, xhigh))
        [x, dx, xlow, xhigh] = compute_statistics(mcmc.trace('P_purity')[:])
        outfile.write('Ppurity:%8.2f +- %8.2f %%            [%8.2f, %8.2f] \n' % (x, dx, xlow, xhigh))
...
if __name__ == "__main__":
    ...
    pool = Pool()
    pool.map(process_datafile, filenames)
